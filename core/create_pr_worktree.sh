...
PR_JSON="$(
  cd "$REPO_DIR" && GH_PROMPT_DISABLED=1 gh pr view "$PR_REF" \
    --json number,headRefName,headRefOid,headRepository,headRepositoryOwner,baseRefName
)"

read -r PR_NUMBER HEAD_REF HEAD_SHA HEAD_REPO_OWNER HEAD_REPO_NAME BASE_REF < <(
...
print(f"{number}\t{head_ref}\t{head_sha}\t{head_owner}\t{head_repo_name}\t{base_ref}")
PY
)
...
if git -C "$REPO_DIR" worktree list --porcelain | awk -v b="refs/heads/$HEAD_REF" '$1=="branch" && $2==b {found=1} END{exit found?0:1}'; then
  LOCAL_BRANCH="${HEAD_REF}-wt-${RUN_ID}"
  if git -C "$REPO_DIR" show-ref --verify --quiet "refs/heads/$LOCAL_BRANCH"; then
    LOCAL_BRANCH="${LOCAL_BRANCH}-$RANDOM"
  fi
fi

if git -C "$REPO_DIR" show-ref --verify --quiet "refs/heads/$LOCAL_BRANCH"; then
  git -C "$REPO_DIR" worktree add "$WORKTREE_DIR" "$LOCAL_BRANCH"
else
  git -C "$REPO_DIR" worktree add -b "$LOCAL_BRANCH" "$WORKTREE_DIR" "$REMOTE_NAME/$HEAD_REF"
fi
