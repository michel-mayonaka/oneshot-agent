...
Describe "core/generate-pr-yml.sh"
  ...
  It "fails when worklog is missing"
    RUN_DIR="$TMP_DIR/run"
    mkdir -p "$RUN_DIR"
    When run bash core/generate-pr-yml.sh "$RUN_DIR"
  ...
  It "fails when worktree is not a git repo"
    ...
    When run env GIT_MOCK_GIT_DIR_STATUS=1 bash core/generate-pr-yml.sh "$RUN_DIR"
    The status should be failure
  ...
  It "generates pr.yml"
    ...
    When run env \
      ...
      bash core/generate-pr-yml.sh "$RUN_DIR"
