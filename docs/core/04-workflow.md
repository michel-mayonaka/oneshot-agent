...
6. `run_dir/report.md` を確認します。

## 例外フロー: AIが間違う
- 症状: 出力が期待と異なります。
- 対応: プロンプトを修正し再実行します。
- 対応: 差分は `run_dir/logs/worklog.md` で確認します。
...
## 例外フロー: 部分失敗
- 症状: 一部成果物のみ生成されます。
- 対応: `run_dir/logs/stderr_and_time.txt` を確認します。
- 対応: 必要なら worktree を削除します。

## 手動復旧手順
- `run_dir` と `run_dir/logs/` を確認します。
- 失敗理由をメモし再実行します。
- worktree 使用時は削除を検討します。
- `core/remove_worktree.sh` を使います。

## Definition of Done
- `run_dir/report.md` が存在します。
- `run_dir/logs/events.jsonl` が存在します。
- `run_dir/prompts/prompt.txt` が存在します。
- `run_dir/logs/stderr_and_time.txt` を確認済みです。
